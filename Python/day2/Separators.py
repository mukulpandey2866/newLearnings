...
# print(s1 + s2 + no) # This line will raise a TypeError because you cannot concatenate a string (s1 + s2) with an integer (no) without converting the integer to a string first. To fix this, you can convert no to a string using str(no):
...
print("Employee Name: {}, ID: {}, Salary: {:.2f}".format(emp_name,emp_id, emp_salary))  # this will print "Employee Name: John Doe, ID: 123, Salary: 50000.00" using positional parameters to specify the order of the variables in the output string.
...
print(f"{emp_name} id is {emp_id} and salary is {emp_salary:.2f}")
